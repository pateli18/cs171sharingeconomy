...
ValueMap.prototype.initVis = function() {
    ...
    vis.harvardIndex = vis.data.length + 1
    var harvardData = {'lat':42.3735695, 'lng':-71.1211549, 'poi':true, 'index':vis.harvardIndex, 'name':'Harvard Square'};
...
ValueMap.prototype.updateValueMap = function() {
    ...
    vis.baseMap.on('moveend', function() {
        ...
        d3.selectAll(".relative-value-map-label")
            .attr("transform", function(d) {
                return "translate("+
                    (vis.baseMap.latLngToLayerPoint([d.lat, d.lng]).x + 8) +","+
                    (vis.baseMap.latLngToLayerPoint([d.lat, d.lng]).y - 8) +")";
            })
            .text(function(d) {
                if (d.poi) {
